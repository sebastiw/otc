...
codec({SecurityHeaderType, Bin}, Opts) when is_atom(SecurityHeaderType), is_binary(Bin) ->
    decode({SecurityHeaderType, Bin}, Opts);
...
decode({plain_nas_message, <<MT:8/big, OIE/binary>>}, Opts) ->
    MsgType = otc_nas_eps:parse_msg_type(MT),
    Msg = decode_emm_msg(MsgType, OIE, Opts),
    Msg#{message_type => MsgType};
decode({service_request, Bin}, Opts) ->
    Msg = decode_emm_msg(service_request, Bin, Opts),
    Msg#{message_type => service_request};
decode({_SHT, <<MAC:4/binary, SN:1/binary, NMSG/binary>>}, _Opts) ->
...
encode({#{message_authentication_code := MAC, sequence_number := SN}, NMSG}, _Opts) ->
    <<MAC:4/binary, SN:1/binary, NMSG/binary>>;
encode({#{message_type := service_request} = Msg, _}, Opts) ->
    encode_emm_msg(service_request, Msg, Opts);
encode({#{message_type := MsgType} = Msg, _}, Opts) ->
    Bin = encode_emm_msg(MsgType, Msg, Opts),
    MT = otc_nas_eps:compose_msg_type(MsgType),
    <<MT:8/big, Bin/binary>>.

decode_emm_msg(attach_accept, Bin0, _Opts) ->
...
decode_emm_msg(attach_complete, Bin0, _Opts) ->
...
decode_emm_msg(attach_reject, Bin0, _Opts) ->
...
decode_emm_msg(attach_request, Bin0, _Opts) ->
...
decode_emm_msg(authentication_failure, Bin0, _Opts) ->
    {EmmCause, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
    Opts = [{authentication_failure_parameter, 16#30, tlv, 16}],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
    Optionals#{emm_cause => EmmCause
              };
decode_emm_msg(authentication_reject, Bin0, _Opts) ->
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(authentication_request, Bin0, _Opts) ->
    {_, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(authentication_response, Bin0, _Opts) ->
...
decode_emm_msg(cs_service_notification, Bin0, _Opts) ->
...
decode_emm_msg(detach_accept, Bin0, _Opts) ->
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(detach_request, Bin0, #{direction := 'id-downlinkNASTransport'}) ->
    %% UE terminated
    {NasKeySetIdentifier, Bin1} = otc_l3_codec:decode_v(Bin0, half),
    {DetachType, Bin2} = otc_l3_codec:decode_v(Bin1, half),
    Opts = [{emm_cause, 16#53, tv, 2},
            {lower_bound_timer_value, 16#1C, tlv, 3},
            {forbidden_roaming_list, 16#1D, tlv, {9, 98}},
            {forbidden_provision_list, 16#1E, tlv, {9, 98}}
           ],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin2, Opts),
    Optionals#{direction => ue_terminated,
               detach_type => DetachType,
               nas_key_set_identifier => NasKeySetIdentifier};
decode_emm_msg(detach_request, Bin0, #{direction := 'id-uplinkNASTransport'}) ->
    %% UE originating
    {NasKeySetIdentifier, Bin1} = otc_l3_codec:decode_v(Bin0, half),
    {DetachType, Bin2} = otc_l3_codec:decode_v(Bin1, half),
    {EpsMobileIdentity, Bin3} = otc_l3_codec:decode_lv(Bin2),
    ...
    Optionals#{direction => ue_originated,
               detach_type => DetachType,
               nas_key_set_identifier => NasKeySetIdentifier,
               eps_mobile_identity => EpsMobileIdentity};
decode_emm_msg(detach_request, Bin0, _Opts) ->
    %% This will be a bit weird:
    %% DETACH REQUEST can come from either network or UE
    %% 1. if it is UE originating detach, then it can only contain the
    %% EPS mobile identity, which is mandatory, but
    %% 2. if it is UE terminated detach, then there are optional fields for
    %% - EMM cause,
    %% - Lower bound timer value,
    %% - TAI list containing forbidden tracking areas for roaming,
    %% - TAI list containing forbidden tracking areas for regional
    ...
    {Optionals, Unknown0} = otc_l3_codec:decode_iei_list(Bin2, Opts),
    case {map_size(Optionals) > 0, byte_size(Unknown0) > 0} of
        {false, true} ->
            %% UE originating detach
            {EpsMobileIdentity, _} = otc_l3_codec:decode_lv(Unknown0),
            #{direction => ue_originated,
              detach_type => DetachType,
              nas_key_set_identifier => NasKeySetIdentifier,
              eps_mobile_identity => EpsMobileIdentity};
        {false, false} ->
            %% No clue; say it's UE terminated detach without any optionals
            Optionals#{direction => ue_terminated,
                       detach_type => DetachType,
                       nas_key_set_identifier => NasKeySetIdentifier};
        {true, false} ->
            ...
            Optionals#{direction => ue_terminated,
                       ...
                       nas_key_set_identifier => NasKeySetIdentifier};
        {true, true} ->
            %% No clue; say it's UE terminated detach with unknown optionals
            Optionals#{direction => ue_terminated,
                       detach_type => DetachType,
                       nas_key_set_identifier => NasKeySetIdentifier}
    end;
decode_emm_msg(downlink_nas_transport, Bin0, _Opts) ->
    {NasMessageContainer, Bin1} = otc_l3_codec:decode_lv(Bin0),
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
    Optionals#{nas_message_container => NasMessageContainer
              };
decode_emm_msg(emm_information, Bin0, _Opts) ->
    ...
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(emm_status, Bin0, _Opts) ->
    {EmmCause, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
    Optionals#{emm_cause => EmmCause
              };
decode_emm_msg(extended_service_request, Bin0, _Opts) ->
    {NasKeySetIdentifier, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(guti_reallocation_command, Bin0, _Opts) ->
...
decode_emm_msg(guti_reallocation_complete, Bin0, _Opts) ->
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(identity_request, Bin0, _Opts) ->
    {_, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(identity_response, Bin0, _Opts) ->
...
decode_emm_msg(security_mode_command, Bin0, _Opts) ->
...
decode_emm_msg(security_mode_complete, Bin0, _Opts) ->
    Opts = [{imeisv, 16#23, tlv, 11},
            {replayed_nas_message_container, 16#79, tlve, {3, n}},
            {ue_radio_capability_id, 16#66, tlv, {3, n}}],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(security_mode_reject, Bin0, _Opts) ->
    {EmmCause, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
    Optionals#{emm_cause => EmmCause
              };
decode_emm_msg(security_protected_nas_message, Bin0, _Opts) ->
...
decode_emm_msg(service_reject, Bin0, _Opts) ->
    {EmmCause, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
...
decode_emm_msg(service_request, Bin0, _Opts) ->
...
decode_emm_msg(tracking_area_update_accept, Bin0, _Opts) ->
    {_, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(tracking_area_update_complete, Bin0, _Opts) ->
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals;
decode_emm_msg(tracking_area_update_reject, Bin0, _Opts) ->
    {EmmCause, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
    Opts = [{t3346_value, 16#5F, tlv, 3},
            {extended_emm_cause, 16#A, tv, 1}],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
...
decode_emm_msg(tracking_area_update_request, Bin0, _Opts) ->
    {NasKeySetIdentifier, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(uplink_nas_transport, Bin0, _Opts) ->
    {NasMessageContainer, Bin1} = otc_l3_codec:decode_lv(Bin0),
    Opts = [],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin1, Opts),
    Optionals#{nas_message_container => NasMessageContainer
              };
decode_emm_msg(downlink_generic_nas_transport, Bin0, _Opts) ->
...
decode_emm_msg(uplink_generic_nas_transport, Bin0, _Opts) ->
    {GenericMessageContainerType, Bin1} = otc_l3_codec:decode_v(Bin0, 1),
    {GenericMessageContainer, Bin2} = otc_l3_codec:decode_lve(Bin1),
    Opts = [{additional_information, 16#65, tlv, {3, n}}],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin2, Opts),
...
decode_emm_msg(control_plane_service_request, Bin0, _Opts) ->
    {NasKeySetIdentifier, Bin1} = otc_l3_codec:decode_v(Bin0, half),
...
decode_emm_msg(service_accept, Bin0, _Opts) ->
    Opts = [{eps_bearer_context_status, 16#57, tlv, 4},
            {t3448_value, 16#6B, tlv, 3}],
    {Optionals, _Unknown} = otc_l3_codec:decode_iei_list(Bin0, Opts),
    Optionals.
...
encode_emm_msg(attach_accept, #{eps_attach_result := EpsAttachResult,
                                t3412_value := T3412Value,
                                tai_list := TaiList,
                                esm_message_container := EsmMessageContainer} = Msg, _Opts) ->
...
encode_emm_msg(attach_complete, #{esm_message_container := EsmMessageContainer} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_lve(EsmMessageContainer, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(attach_reject, #{emm_cause := EmmCause} = Msg, _Opts) ->
...
encode_emm_msg(attach_request, #{eps_attach_type := EpsAttachType,
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, Bin3/bitstring, Bin4/bitstring, Bin5/bitstring, OptBin/binary>>;
encode_emm_msg(authentication_failure, #{emm_cause := EmmCause} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(EmmCause, 1, <<>>),
    Opts = [{authentication_failure_parameter, 16#30, tlv, 16}],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(authentication_reject, #{} = Msg, _Opts) ->
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(authentication_request, #{nas_key_set_identifierasme := NasKeySetIdentifierasme,
                                         authentication_parameter_rand_eps_challenge := AuthenticationParameterRandEpsChallenge,
                                         authentication_parameter_autn_eps_challenge := AuthenticationParameterAutnEpsChallenge} = Msg, _Opts) ->
    Bin1 = <<0:4>>,  % spare_half_octet
...
encode_emm_msg(authentication_response, #{authentication_response_parameter := AuthenticationResponseParameter} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_lv(AuthenticationResponseParameter, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(cs_service_notification, #{paging_identity := PagingIdentity} = Msg, _Opts) ->
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(detach_accept, #{} = Msg, _Opts) ->
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(detach_request, #{detach_type := DetachType,
                                 nas_key_set_identifier := NasKeySetIdentifier,
                                 eps_mobile_identity := EpsMobileIdentity} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(NasKeySetIdentifier, half, <<>>),
...
encode_emm_msg(detach_request, #{detach_type := DetachType} = Msg, _Opts) ->
    Bin1 = <<0:4>>,  % spare_half_octet
    Bin2 = otc_l3_codec:encode_v(DetachType, half, <<>>),
    Opts = [{emm_cause, 16#53, tv, 2},
            {lower_bound_timer_value, 16#1C, tlv, 3},
...
encode_emm_msg(downlink_nas_transport, #{nas_message_container := NasMessageContainer} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_lv(NasMessageContainer, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(emm_information, #{} = Msg, _Opts) ->
    Opts = [{full_name_for_network, 16#43, tlv, {3, n}},
            {short_name_for_network, 16#45, tlv, {3, n}},
            {local_time_zone, 16#46, tv, 2},
            {universal_time_and_local_time_zone, 16#47, tv, 8},
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(emm_status, #{emm_cause := EmmCause} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(EmmCause, 1, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(extended_service_request, #{service_type := ServiceType,
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, Bin3/bitstring, OptBin/binary>>;
encode_emm_msg(guti_reallocation_command, #{guti := Guti} = Msg, _Opts) ->
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(guti_reallocation_complete, #{} = Msg, _Opts) ->
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(identity_request, #{identity_type := IdentityType} = Msg, _Opts) ->
    Bin1 = <<0:4>>,  % spare_half_octet
...
encode_emm_msg(identity_response, #{identity_response_message := IdentityResponseMessage,
                                    mobile_identity := MobileIdentity} = Msg, _Opts) ->
...
encode_emm_msg(security_mode_command, #{selected_nas_security_algorithms := SelectedNasSecurityAlgorithms,
                                        nas_key_set_identifier := NasKeySetIdentifier,
                                        replayed_ue_security_capabilities := ReplayedUeSecurityCapabilities} = Msg, _Opts) ->
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, Bin3/bitstring, Bin4/bitstring, OptBin/binary>>;
encode_emm_msg(security_mode_complete, #{} = Msg, _Opts) ->
    Opts = [{imeisv, 16#23, tlv, 11},
            {replayed_nas_message_container, 16#79, tlve, {3, n}},
            {ue_radio_capability_id, 16#66, tlv, {3, n}}],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(security_mode_reject, #{emm_cause := EmmCause} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(EmmCause, 1, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(security_protected_nas_message, #{message_authentication_code := MessageAuthenticationCode,
                                                 sequence_number := SequenceNumber,
                                                 nas_message := NasMessage} = Msg, _Opts) ->
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, Bin3/bitstring, OptBin/binary>>;
encode_emm_msg(service_reject, #{emm_cause := EmmCause} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(EmmCause, 1, <<>>),
    Opts = [{t3346_value, 16#5F, tlv, 3},
            {t3448_value, 16#6B, tlv, 3}],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
...
encode_emm_msg(service_request, #{ksi_and_sequence_number := KsiAndSequenceNumber,
                                  message_authentication_code_short := MessageAuthenticationCodeShort} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(KsiAndSequenceNumber, 1, <<>>),
    Bin2 = otc_l3_codec:encode_v(MessageAuthenticationCodeShort, 2, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, OptBin/binary>>;
encode_emm_msg(tracking_area_update_accept, #{eps_update_result := EpsUpdateResult} = Msg, _Opts) ->
    Bin1 = <<0:4>>,  % spare_half_octet
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, OptBin/binary>>;
encode_emm_msg(tracking_area_update_complete, #{} = Msg, _Opts) ->
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin;
encode_emm_msg(tracking_area_update_reject, #{emm_cause := EmmCause} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(EmmCause, 1, <<>>),
    Opts = [{t3346_value, 16#5F, tlv, 3},
            {extended_emm_cause, 16#A, tv, 1}],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
...
encode_emm_msg(tracking_area_update_request, #{eps_update_type := EpsUpdateType,
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, Bin3/bitstring, OptBin/binary>>;
encode_emm_msg(uplink_nas_transport, #{nas_message_container := NasMessageContainer} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_lv(NasMessageContainer, <<>>),
    Opts = [],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, OptBin/binary>>;
encode_emm_msg(downlink_generic_nas_transport, #{generic_message_container_type := GenericMessageContainerType,
                                                 generic_message_container := GenericMessageContainer} = Msg, _Opts) ->
...
encode_emm_msg(control_plane_service_request, #{control_plane_service_type := ControlPlaneServiceType,
                                                nas_key_set_identifier := NasKeySetIdentifier} = Msg, _Opts) ->
    Bin1 = otc_l3_codec:encode_v(NasKeySetIdentifier, half, <<>>),
    ...
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    <<Bin1/bitstring, Bin2/bitstring, OptBin/binary>>;
encode_emm_msg(service_accept, #{} = Msg, _Opts) ->
    Opts = [{eps_bearer_context_status, 16#57, tlv, 4},
            {t3448_value, 16#6B, tlv, 3}],
    OptBin = otc_l3_codec:encode_iei_list(Msg, Opts),
    OptBin.
