...
encode_parameters(Msg, Parameters, Opts) ->
    encode_parameters(Msg, Opts, Parameters, <<>>).

encode_parameters(_, _, [], Acc) ->
    Acc;
encode_parameters(_, _, [{end_of_optional_parameters, _}|_], Acc) when byte_size(Acc) > 0 ->
    <<Acc/binary, ?SCCP_IEI_END_OF_OPTIONAL_PARAMETERS:8/big>>;
encode_parameters(Msg, Opts, [{Name, _}|Os], Acc) when is_map_key(Name, Msg) ->
    V = maps:get(Name, Msg),
    Bin = encode_parameter(Name, V, Opts),
    IEI = compose_iei(Name),
    Len = byte_size(Bin),
    NAcc = <<IEI:8/big, Len:8/big, Bin/binary, Acc/binary>>,
    encode_parameters(Msg, Opts, Os, NAcc);
encode_parameters(Msg, Opts, [_|Os], Acc) ->
    encode_parameters(Msg, Opts, Os, Acc).
